      ...
      <section className="container mx-auto px-4 pb-32 min-h-[50vh]">
        {loading ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-full border-[3px] border-stone-200 rounded-[2rem] bg-stone-50 overflow-hidden flex flex-col">
                <Skeleton className="w-full aspect-[4/3] bg-stone-200" />
                <div className="p-8 flex flex-col gap-4">
                  <div className="flex justify-center mb-2">
                    <Skeleton className="h-8 w-3/4 bg-stone-200 rounded-lg" />
                  </div>
                  <Skeleton className="h-4 w-full bg-stone-200 rounded" />
                  <Skeleton className="h-4 w-2/3 bg-stone-200 rounded" />
                  ...
                  <Skeleton className="h-12 w-full mt-6 bg-stone-200 rounded-xl" />
                </div>
              </div>
            ))}
          </div>
                ...
                <motion.div key={branch.id} variants={fadeIn} className="group h-full">
                  ...
                  >
                    ...
                    <div className="relative aspect-[4/3] w-full overflow-hidden bg-stone-100 border-b-[3px] border-stone-900">
                      {branch.image_url ? (
                        <Image
                          // Optimize the image: Request 600px width (enough for grid), 80% quality
                          src={getOptimizedImage(branch.image_url, 600, 80)}
                          alt={branch.name}
                          fill
                    ...
                    <CardContent className="p-8 flex flex-col flex-1 relative">
                      ...
                      {/* Buton (Eskiz tarzı) */}
                      <Link href={`/sube-detay?slug=${branch.slug}`} className="mt-auto">
                        <Button
