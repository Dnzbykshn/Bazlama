...
function BranchDetailContent() {
    ...
    if (!slug) return <ErrorState error="Şube seçilmedi." />;
    ...
    if (loading) {
        return (
            <main className="min-h-screen bg-[#fffcf8]">
                <Header />
                {/* Hero Skeleton */}
                <div className="relative h-[70vh] min-h-[500px] bg-stone-900 flex items-end justify-center pb-24">
                    <div className="container mx-auto px-4 max-w-4xl space-y-6 flex flex-col items-center z-10">
                        <Skeleton className="h-4 w-24 bg-white/10 rounded" />
                        <Skeleton className="h-8 w-32 bg-white/10 rounded-full" />
                        <Skeleton className="h-20 w-3/4 bg-white/10 rounded-xl" />
                        <Skeleton className="h-6 w-1/2 bg-white/10 rounded" />
                    </div>
                </div>
                {/* Content Skeleton */}
                <div className="relative -mt-20 bg-[#fffcf8] rounded-t-[3rem] px-4 pb-32">
                    <div className="container mx-auto pt-24 space-y-16 max-w-6xl">
                        {/* Info Cards Skeleton */}
                        <div className="grid md:grid-cols-2 gap-8">
                            <Skeleton className="h-80 w-full rounded-[3rem] bg-stone-200" />
                            <Skeleton className="h-80 w-full rounded-[3rem] bg-stone-200" />
                        </div>
                        {/* Gallery Skeleton */}
                        <Skeleton className="h-[400px] w-full rounded-[3rem] bg-stone-200" />
                    </div>
                </div>
            </main>
        )
        ...
        <main className={`min-h-screen bg-[#fffcf8] selection:bg-[#8AD7D6] selection:text-white ${inter.className}`}>
            ...
            <section className="relative h-[70vh] min-h-[500px] flex items-end justify-center pb-24 text-center text-white overflow-hidden">
                ...
                <div className="absolute inset-0 z-0">
                    <Image
                        // Hero için: 1200px genişlik, %85 kalite
                        src={getOptimizedImage(branch.image_url || "/placeholder-branch.jpg", 1200, 85)}
                        alt={branch.name}
                        fill
                            ...
                            <div className="relative max-w-5xl mx-auto group">
                                <div className="relative aspect-[16/9] md:aspect-[21/9] w-full rounded-[3rem] overflow-hidden shadow-2xl shadow-stone-200/40 border-[6px] border-white">
                                    <AnimatePresence mode="wait">
                                        ...
                                        >
                                            <Image
                                                // Galeri slider: 1200px genişlik
                                                src={getOptimizedImage(galleryItems[currentImageIndex].image_url, 1200, 85)}
                                        ...
                                        {items.map((item) => (
                                            <motion.div key={item.id} whileHover={{ x: 5 }} className="flex gap-6 items-start group p-4 rounded-2xl hover:bg-white transition-all duration-300">
                                                <div className="relative w-24 h-24 flex-shrink-0 rounded-2xl overflow-hidden bg-stone-100 shadow-sm">
                                                    {item.image_url ? (
                                                        <Image
                                                            // Menu item: 200px thumbnail
                                                            src={getOptimizedImage(item.image_url, 200, 80)}
                                                            alt={item.title}
                                                            fill
...
export default function BranchDetailPage() {
    return (
        <Suspense fallback={
            // Fallback for Suspense itself (e.g. before searchParams are ready)
            <main className="min-h-screen bg-[#fffcf8] flex items-center justify-center">
                <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#8AD7D6]"></div>
            </main>
        }>
            <BranchDetailContent />
        </Suspense>
